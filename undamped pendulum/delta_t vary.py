...
if __name__ == '__main__':
    ...
    # different step size
    delta_t = [0.001,0.02,0.035]

    for k in range(3):
        # time sequence
        t = list(np.arange(0,5*T,delta_t[k],dtype='float32'))
        t = t/T
        ...
        # Solve!
        [theta_list,d_theta_list] = _Undamped_Pendulum_.Euler_solve(_Undamped_Pendulum_,t,delta_t[k],theta_0,d_theta_0)
        theta_list = theta_list * 180 / math.pi
    
        plt.figure(1)
        plt.title("Graph of " r'$\theta(t)$' " at small angles with different " r'$\Delta t$')
        plt.xlabel("dimensionless time  t/" r'$\tau$')
        plt.ylabel(r'$\theta$' "$(\mathrm{degree})$")
        plt.plot(t,theta_list)
        plt.grid()

plt.legend(['0.001','0.02','0.035'],loc='upper left',title="$\Delta t\;(\mathrm{s})$")
plt.show()